...
if (empty($id_formulario) || empty($respostas)) {
    echo "
    <!DOCTYPE html>
    <html lang='pt-BR'>
    <head>
        ...
        <title>Erro</title>
        <script>
            alert('Dados incompletos para processar as respostas.');
            window.location.href = 'visualizarRespostas.php?id=" . urlencode($id_formulario) . "';
        </script>
    </head>
    <body>
    </body>
...
try {
        ...
        if (is_array($resposta)) { // Para múltipla escolha
            foreach ($resposta as $id_resposta) {
                ...
                if ($result_verificar_resposta->num_rows === 0) {
                    throw new Exception("Erro: Resposta inválida para a pergunta $id_pergunta.");
                }
    ...
    <head>
        ...
        <title>Erro</title>
        <script>
            alert('Ocorreu um erro ao processar as respostas: " . addslashes($e->getMessage()) . "');
            window.location.href = 'visualizarRespostas.php?id=" . urlencode($id_formulario) . "';
        </script>
    </head>
    <body>
